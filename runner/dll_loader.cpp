...
namespace
{
HMODULE g_dwpose_worker_module = nullptr;
...
bool ensure_libraries_loaded(std::string* err_out)
{
    ...
    *slash = L'\0';
    ...
    if(!h)
    {
        ...
        TDDBG("!! LoadLibraryEx dwpose_worker.dll failed: " << narrow(msg.c_str()));
        if(err_out)
            *err_out = "LoadLibraryEx dwpose_worker.dll: " + narrow(msg.c_str());
        return false;
    }

    g_dwpose_worker_module = h;
    TDDBG("ensure_libraries_loaded: dwpose_worker.dll loaded OK");
    return true;
}
